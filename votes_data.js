...
// ✅ ALGO CORRIGÉ: Mapper chaque question au scrutin correct
function calculerAffiniteParti(reponses, parti, questions) {
  let totalAccord = 0;
  let totalVotes = 0;
  
  questions.forEach((question, qIdx) => {
    const reponseUser = reponses[qIdx];
    if (reponseUser === null || reponseUser === 3) return; // Skip pas répondu
    
    const voteUser = mapIndexToVote(reponseUser);
    const scrutinId = question.scrutin;
    const scrutin = SCRUTINS.find(s => s.id === scrutinId);
    
    if (!scrutin) {
      console.warn("Scrutin introuvable pour", scrutinId);
      return;
    }
    
    // Compter les accords du parti pour ce scrutin
    Object.values(scrutin.personnages).forEach(perso => {
      if (perso.groupe === parti) {
        const votePerso = perso.votes[qIdx % perso.votes.length]; // Même index dans les votes du scrutin
        if (voteUser === votePerso) {
          totalAccord++;
        }
        totalVotes++;
      }
...
function calculerAffinitePersonnage(reponses, perso, questions) {
  let totalAccord = 0;
  let totalVotes = 0;
  
  questions.forEach((question, qIdx) => {
    const reponseUser = reponses[qIdx];
    if (reponseUser === null || reponseUser === 3) return;
    
    const voteUser = mapIndexToVote(reponseUser);
    const scrutinId = question.scrutin;
    const scrutin = SCRUTINS.find(s => s.id === scrutinId);
    
    if (!scrutin || !scrutin.personnages[perso]) return;
    
    const votePerso = scrutin.personnages[perso].votes[qIdx % scrutin.personnages[perso].votes.length];
    if (voteUser === votePerso) {
      totalAccord++;
    }
    totalVotes++;
