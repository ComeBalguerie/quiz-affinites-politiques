...
// SCRUTINS (7 scrutins officiels + positional votes pour 45 questions supplémentaires)
const SCRUTINS = [
  {
    id: "immigration",
    titre: "Immigration",
...
// CALCUL AFFINITÉS (reste identique)
    ...
    Object.values(scrutin.personnages).forEach(perso => {
      if (perso.groupe === parti) {
        for (let i = 0; i < perso.votes.length; i++) {
          const reponseUser = reponses[scrutinIdx * (scrutin === SCRUTINS[0] ? 7 : scrutin === SCRUTINS[1] ? 8 : scrutin === SCRUTINS[2] ? 9 : scrutin === SCRUTINS[3] ? 6 : scrutin === SCRUTINS[4] ? 6 : scrutin === SCRUTINS[5] ? 3 : 2) + i];
